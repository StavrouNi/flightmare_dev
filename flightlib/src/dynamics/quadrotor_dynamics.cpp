...
bool QuadrotorDynamics::updateParams(const YAML::Node& params) {
  if (params["quadrotor_dynamics"]) {
    ...
    // Row 0 is X-coordinates
    t_BM_.row(0) << tbm_fl[0], tbm_fr[0], tbm_br[0], tbm_bl[0];

    // Row 1 is Y-coordinates
    t_BM_.row(1) << tbm_fl[1], tbm_fr[1], tbm_br[1], tbm_bl[1];

    // Row 2 is Z-coordinates
    t_BM_.row(2) << tbm_fl[2], tbm_fr[2], tbm_br[2], tbm_bl[2];
    ...
    // Ensure the Yaw signs match FL, FR, BR, BL order
    // Standard X-Quad: FL(CW)=-1, FR(CCW)=+1, BR(CW)=-1, BL(CCW)=+1
    // Note: Verify if your FL is CW or CCW. If FL is CCW, flip the signs.
    B_allocation_ =
      (Matrix<4, 4>() << Vector<4>::Ones().transpose(), t_BM_.row(1),
      -t_BM_.row(0), kappa_ * Vector<4>(-1.0, 1.0, -1.0, 1.0).transpose())
        .finished();
